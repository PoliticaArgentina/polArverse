...
Con la ayuda del :package: `polArViz` podemos acceder rápidamente a una serie de visualizaciones de los datos electorales.
...
Este nivel de agregación nos permite también utilizar la función `plot_results()`. La misma, al detectar que se trata de datos al nivel de `departamentos`  hará uso de grillas predefinidas por el paquete `{geoAr}` para ser usadas por `geofacet::facet_grid()`
